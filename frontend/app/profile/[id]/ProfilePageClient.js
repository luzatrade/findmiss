...
const ProfilePageClient = ({ id }) => {
  ...
  const handleShare = async () => {
    if (!profile) return
    const ok = await shareProfile({ id: profile.id, name: profile.name })
    setShareMessage(ok ? 'Link copiato negli appunti' : 'Impossibile condividere')
  ...
  const handleOpenChat = () => {
    ...
    if (!token) {
      router.push(`/auth?redirect=${encodeURIComponent(`/chat?announcement=${profile.id}`)}`)
      return
    }
    router.push(`/chat?announcement=${profile.id}`)
  }

  const handleReport = () => {
    window.location.href = 'mailto:support@findmiss.it?subject=Segnalazione profilo'
  }
          ...
          <div className="relative w-full h-56 sm:h-72 lg:h-80 overflow-hidden rounded-b-2xl shadow-[0_16px_40px_rgba(0,0,0,0.45)]">
            ...
            <div className="absolute bottom-4 left-4 right-4 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
              ...
              <div className="flex flex-wrap gap-2 justify-start sm:justify-end">
                ...
                <button
                  onClick={handleShare}
                  className="inline-flex items-center gap-1 px-4 py-2 rounded-full text-sm font-medium shadow-md bg-white/95 text-gray-900 border border-pink-100 hover:bg-pink-50"
                >
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 px-4 py-2 rounded-full text-sm font-medium shadow-md bg-pink-500 text-white hover:bg-pink-600"
                >
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 space-y-3">
              ...
              <div className="space-y-2 text-sm text-gray-700">
                ...
                {profile.phone ? (
                  ...
                  <p className="text-xs text-gray-500">Contatto disponibile via chat</p>
                )}
              </div>
            </div>
      ...
      <footer className="sticky bottom-0 z-30 bg-white border-t border-gray-200 shadow-[0_-1px_8px_rgba(0,0,0,0.04)]">
        <div className="max-w-6xl mx-auto px-4 py-3 flex flex-col sm:flex-row gap-2 sm:gap-3 items-stretch sm:items-center justify-between">
          ...
          <div className="flex flex-1 sm:flex-none justify-between sm:justify-end gap-2">
            ...
            <button
              onClick={handleShare}
              className="hidden sm:inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gray-100 text-gray-700 text-sm font-medium border border-gray-200 hover:bg-gray-200 transition"
            >
              <Share2 className="w-4 h-4" />
              Condividi
            </button>
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gray-100 text-gray-600 text-xs sm:text-sm font-medium border border-gray-200 hover:bg-gray-200 transition"
            >
              Segnala
            </button>
          </div>
